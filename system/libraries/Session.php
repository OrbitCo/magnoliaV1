...
class CI_Session
{
    ...
    public function __construct($params = [])
    {
        ...
        foreach (['sess_encrypt_cookie', 'sess_use_database', 'sess_table_name', 'api_table_name', 'sess_expiration', 'token_expiration', 'sess_match_ip', 'sess_match_useragent', 'sess_cookie_name', 'cookie_path', 'cookie_domain', 'sess_time_to_update', 'time_reference', 'cookie_prefix', 'encryption_key'] as $key) {
    ...
    public function sess_write()
    {
        ...
        $this->ci->db->where('session_id', $session_id)
            ->update($this->sess_table_name, $session_data);
        $this->ci->cache->delete('sessions', $session_id);
    ...
    public function sess_create_token()
    {
        if ($this->ci->pg_module->is_module_active('get_token')) {
            ...
            if (!empty($result) && intval($result[0]->cnt) > 0) {
                $this->userdata['user_data'] = serialize(['token_id' => serialize($result[0]->id)]);
            ...
            $this->userdata['user_data'] = serialize(['token_id' => $this->ci->db->insert_id()]);
    ...
    {
        if ($this->ci->pg_module->is_module_active('get_token')) {
            $token = $token ?: trim(strip_tags($this->ci->input->get_post('token', true)));
