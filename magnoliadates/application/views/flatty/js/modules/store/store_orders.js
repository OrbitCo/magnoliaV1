...
function storeOrders(optionArr)
{
    ...
    this.Init = function (options) {
        ...
        if (_self.properties.order.status == 'status_confirmed_recipient') {
            ...
            _self.shippingMethodsLoad(0, user_id, function () {
                _self.isOneshippingMethods();
            });
        }
        if (_self.properties.data_order.for_friend || _self.properties.data_order.for_myself) {
            _self.loadOrderDaeta();
        }
        _order_data.comment = _self.properties.order.comment;
    };
    ...
    this.init_controls = function () {
        $(_self.properties.commonAncestor).off('click', _self.properties.forFriendBlock).on('click', _self.properties.forFriendBlock, function () {
            ...
            _self.addressChoose($(this).data('addressid'), function () {
                _self.isOneshippingMethods();
            });
    ...
    this.addressLoad = function (load) {
        if (load) {
            $(_self.properties.addressBlock).show();
            if ($(_self.properties.lastAddress).length === 1) {
                _self.addressChoose(
                    $(_self.properties.lastAddress).data('addressid'),
                    function () {
                        _self.isOneshippingMethods();
                    }
                  );
            ...
            $(_self.properties.addressBlock).hide();
            if (_self.properties.order.status !== 'status_confirmed_recipient') {
                $(_self.properties.shippingMethods).html('');
            }
        }
    };

    this.loadUsers = function () {
        _request(_self.properties.changeUserLink, '', '', 'JSON', function (response) {
            _self.showLoadBlock(response.data);
            _self.friendsList();
            _self.mySentGifts();
        });
    };
    ...
    this.saveAddress = function (id) {
        ...
        _request(_self.properties.saveAddressLink, _self.properties.order.id_order + "/" + id, data, 'json', function (data) {
            ...
            if (result['address_id']) {
                ...
                _self.addressChoose(result['address_id'], function () {
                    _self.isOneshippingMethods();});
    ...
    this.changeRegion = function (value) {
        ...
        _self.properties.timeout_obj = setTimeout(function () {
            ...
            ;
            var data = {'country': country, 'data': value, 'type': 'region'};
            _request(_self.properties.regionCityLoadLink, '', data, 'json', function (data) {
                var result = data;
                if (typeof result['msg'] !== 'undefined') {
                    $(_self.properties.regionBlock + ' ul').append('<li>' + result['msg'] + '</li>');
                } else {
    ...
    this.changeCity = function (value) {
        ...
        _self.properties.timeout_obj = setTimeout(function () {
            ...
            var data = {'country': country, 'region': region, 'data': value, 'type': 'city'};
            _request(_self.properties.regionCityLoadLink, '', data, 'json', function (data) {
                var result = data;
                if (typeof result['msg'] !== 'undefined') {
                    $(_self.properties.cityBlock + ' ul').append('<li>' + result['msg'] + '</li>');
                } else {
    ...
    this.addressDelete = function (obj) {
        ...
        _request(_self.properties.removeAddressLink, id, '', 'html', function (data) {
            $(_self.properties.addressId + data).hide(function () {
                $(this).remove();
            });
    ...
    this.isOneshippingMethods = function () {
        if ($('.shipping-methods').length === 1) {
            $('#shipping_method_' + $('.shipping-methods').data('shippingid')).addClass('select-shipping');
    ...
    this.cancelOrder = function (obj) {
        ...
        _request(_self.properties.closePreorderLink, order_id, data, 'json', function (data) {
            ...
            if (result.errors) {
                ...
                obj.closest('div.js-order-product').hide(function () {
    ...
    this.viewHistory = function (obj) {
        ...
        if (obj.hasClass('fa-folder-o')) {
            _request(_self.properties.orderLogLink, order_id, '', 'json', function (data) {
                ...
                $(_self.properties.orderLogBlock + order_id).html(result['html']).show();
            });
        } else {
            obj.removeClass('fa-folder-open-o').addClass('fa-folder-o');
            $(_self.properties.orderLogBlock + order_id).hide(function () {
                $(this).html('');
            });
        }
    };
