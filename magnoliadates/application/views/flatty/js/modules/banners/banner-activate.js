function BannerActivate(defOptions)
{
    ...
    this.refresh_sum = function () {
        ...
        $('#' + _self.properties.mainID + ' input').each(function () {
            ...
            if (val > _self.properties.data[used_id].positions) {
                val = _self.properties.data[used_id].positions;
            }
            if (isNaN(val)) {
                val = '';
            }
    ...
    this.initialize_positions = function () {
        $('#' + _self.properties.mainID + ' input').each(function () {
            $(this).on('change keydown keyup blur', function () {
                _self.refresh_sum();
            });
            var used_id = parseInt($(this).attr('id').substring(9));
...
if (typeof exports === 'object') {
    ...
    exports.BannerActivate = BannerActivate;
}
