...
class AdminSocialNetworking extends \Controller
{
    ...
    public function __construct()
    {
        ...
        $this->MenuModel->set_menu_active_item('admin_menu', 'system-items');
    }
    ...
    public function services()
    {
        ...
        $this->MenuModel->set_menu_active_item('admin_social_networking_menu', 'services_list_item');
        ...
        if (!empty($data)) {
            if ($status == 1) {
                ...
                if ($data['oauth_version'] == 2) {
                    ...
                    $result = $this->Social_networking_connections_model->checkOauth2Connection(
                        $data,
                        site_url('admin/social_networking/oauth_active/' . $service_id . '/1/')
                    );
                } else {
                    $result = $this->Social_networking_connections_model->checkOauthConnection(
                        $data,
                        site_url('admin/social_networking/oauth_active/' . $service_id . '/1/')
                    );
                }
    ...
    public function application(int $service_id = null)
    {
        $this->load->model('social_networking/models/Social_networking_services_model');
        $data = $this->Social_networking_services_model->get_service_by_id($service_id);
        if ($data) {
            ...
            $this->view->setHeader($data['name']);
            $data['link'] = $this->Social_networking_services_model->getLinks($service_id);
            $this->view->assign('data', $data);
    ...
    public function widgets($page_id = null)
    {
        ...
        if ($data) {
            ...
            $services = $this->Social_networking_services_model->get_services_list(['id' => 'ASC']);
