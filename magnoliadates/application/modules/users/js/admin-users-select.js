function usersSelected(optionArr)
{
    ...
    this.deleteUser = function (is_module) {
        if (_self.properties.isUserDeleted === 0) {
            $('input[name^=module]').each(function (index,item) {
                if (typeof is_module === 'undefined' || is_module === false) {
                    $(item).iCheck('enable');
                }
            });
...
//                  _self.load_users(search, page);
    ...
    this.reload_selected = function (data) {
        ...
        $('#' + _self.properties.id_selected_items + ' input:checkbox').unbind('click').on('click', function () {
            _self.unset_user($(this).val());
        });
    };
...
if (typeof exports === 'object') {
    ...
    exports.usersSelected = usersSelected;
}
