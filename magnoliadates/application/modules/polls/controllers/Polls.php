...
class Polls extends \Controller
{
    ...
    public function ajaxPoll($poll_id = 0, $one_poll_place = null)
    {
        ...
        echo show_poll_place_block(['id_poll' => $poll_id, 'one_poll_place' => $one_poll_place]);
    }
    ...
    public function ajaxSaveResult()
    {
        ...
        $this->Polls_model->saverespond($respond);
        ...
        echo show_poll_place_block(['id_poll' => $poll['id'], 'one_poll_place' => true]);
    }
    ...
    public function index()
    {
        ...
        $params['where_in']['language'] = ['0', $language];
        $params["where_sql"][]          = ' ((use_expiration = 0 OR (use_expiration = 1 AND date_end >= \'' . date('Y-m-d H:i:s') . '\')) AND date_start < \'' . date('Y-m-d H:i:s') . '\') ';
        if ($auth_type == 'user') {
            $params['where_in']['poll_type'] = ['-1', '0', $user_type];
        } else {
            $params['where_in']['poll_type'] = ['-2', '0'];
        }

        $polls = $this->Polls_model->get_polls_list(null, null, ['date_add' => 'ASC'], $params);
