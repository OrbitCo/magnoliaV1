...
use Pg\Libraries\Acl\Action\ViewPage;
use Pg\Libraries\Acl\Resource\Page;
...
class InstallModel extends \Model
{
    ...
    public function generate_password(): string
    {
        ...
        return substr($str, rand(0, 15), 8);
    }

    public function save_install_config($data): bool
    {
    ...
    public function save_config($data): bool
    {
        ...
        $template = str_replace('[ci_env]', ($_SERVER['CI_ENV'] ?? 'production'), $template);
        ...
        if (!empty($sql_array)) {
            foreach ($sql_array as $sql) {
                ...
                if (!empty($sql)) {
                    ...
                    if (!$result && !preg_match('/^alter/i', $sql) && !preg_match('/^duplicate/i', $this->ci->db->_error_message())) {
    ...
    public function get_installed_modules($order_by = 'module_gid ASC', $params = []): array
    {
        $modules = [];
        if ($order_by) {
            $params['order_by'] = $order_by;
        }
        $modules_by_id = $this->ci->pg_module->get_modules($params) ?? [];
        usort($modules_by_id, (static function ($a, $b) {
    ...
    public function get_depend_modules($module_gid): array
    {
        $modules = [];
    ...
    {
        if (empty($this->modules_config[$module_gid])) {
            $module_config = MODULEPATH . $module_gid . '/install/module.php';
            if (file_exists($module_config)) {
                $module = [];
                require $module_config;
                $this->modules_config[$module_gid] = $module ?: [];
            }
        }

        return $this->modules_config[$module_gid] ?? [];
    }
    ...
    public function has_module_requirements($module_gid): bool
    {
     ...
     * @return bool
     */
    public function execute_method(string $model_name, string $method, $min_version = null, $params = null)
    {
        if (!is_array($method)) {
            $method_name = $method;
     ...
     */
    public function load_install_model(string $module_gid)
    {
        $model_name = ucfirst($module_gid.'_install_model');
     ...
     * @return void
     */
    public function _structure_install(string $module_gid)
    {
        putenv("SET_MODULE_ACTION=$module_gid");
        $this->ci->load->model('install/models/InstallMigrateModel');
        $this->ci->InstallMigrateModel->setModule($module_gid)->execute('migrate', ENVIRONMENT, '0');
    }
     ...
     * @return mixed
     */
    public function _structure_deinstall(string $module_gid)
    {
        putenv("SET_MODULE_ACTION=$module_gid");
        $this->ci->load->model('install/models/InstallMigrateModel');
        $this->ci->InstallMigrateModel->setModule($module_gid)->execute('rollback', ENVIRONMENT, '0');
    }

    public function demo_structure_install($module_gid)
    {
        //TODO (nsavanaev) migration
        putenv("SET_MODULE_ACTION=$module_gid");
        $this->ci->load->model('install/models/InstallMigrateModel');
        $this->ci->InstallMigrateModel->setModule($module_gid)->execute('seed:run', ENVIRONMENT, '0');
    }

    public function demo_structure_deinstall($module_gid)
    {
        //TODO (nsavanaev) migration наверное стоит удалить
//        $module_data = $this->get_module_config($module_gid);
//
...
//        return $this->simple_execute_sql($file);
    ...
    public function _language_install($module_gid, $lang_id = null, $delete_old = true)
    {
        ...
        $last_item = end($installed_langs);

        foreach ($installed_langs as $installed_lang) {
            if (!empty($lang_id) && ($lang_id != $installed_lang['id'])) {
                continue;
            }

            $is_end = $installed_lang['id'] == $last_item['id'];
            ...
            if (file_exists($pages_file)) {
                ...
                if (!empty($install_lang) && is_array($install_lang)) {
                    $this->ci->pg_language->pages->set_strings($module_gid, $install_lang, $installed_lang['id'], $is_end);
                    if (!$default_lang_installing) {
                        $this->ci->pg_language->pages->set_strings($module_gid, $install_lang, $default_lang_id, $is_end);
                    }
                } else {
            ...
            if (file_exists($ds_file)) {
                ...
                if (!empty($install_lang) && is_array($install_lang)) {
                    ...
                    $last_ds_key = key($install_lang);
                    foreach ($install_lang as $ref_gid => $value) {
                        $is_ds_end = $is_end && ($last_ds_key == $ref_gid);
                        $this->ci->pg_language->ds->set_module_reference($module_gid, $ref_gid, $value, $installed_lang['id'], $is_ds_end);
                        if (!$default_lang_installing) {
                            $this->ci->pg_language->ds->set_module_reference($module_gid, $ref_gid, $value, $default_lang_id, $is_ds_end);
                        }
                    }
                } else {
    ...
    private function addAclPermissions($module_gid, array $permissions)
    {
        ...
        $subscription_type = $this->ci->Access_permissions_settings_model
            ->getSubscriptionType(AccessPermissionsSettingsModel::TYPE);
    ...
    public function generate_module_lang_install($modules = null, $langs = null, $filename = 'lang_install', $unit_test = false)
    {
        ...
        foreach ($modules as $module_gid) {
            ...
            if ($config && isset($config['linked_modules']) && $config['linked_modules']) {
     ...
     * @return array
     */
    public function get_available_langs(): array
    {
    ...
    //TODO (nsavanaev) удалитиь после приведения к SR
