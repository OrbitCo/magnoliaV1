...
function fieldEditorSelect(optionArr)
{
    ...
    this.set_sortable = function () {
        ...
        $('#' + _self.properties.itemsUlID + ' > li').each(function () {
            ...
            $(this).find('.edit_link').on('click', function () {
                _self.get_option_form(option_gid);
                return false;
            });

            $(this).find('.delete_link').on('click', function () {
                _self.delete_option(option_gid);
                return false;
            });

            $(this).find('.active_link').on('click', function () {
                _self.set_default_option(option_gid, 'activate');
                return false;
            });

            $(this).find('.deactive_link').on('click', function () {
    ...
    this.get_option_form = function (option_gid) {
        $.ajax({
            ...
            cache: false,
            success: function (data) {
                let id_modal = _self.properties.contentObj.show_load_block(data);
                let close_btn_id = id_modal.replace('user_load_content', 'user_load_content_close');
                $('#' + id_modal + '_footer').hide();
                $('#' + id_modal + ' .js-btn-actions').append($('#' + close_btn_id).clone().prop('id', close_btn_id + '_in_content'));

                $( '#' + close_btn_id + '_in_content').off('click').click(function () {
                    _self.properties.contentObj.hide_load_block();
                    return false;
                });
                $('#' + _self.properties.btnCancelID).off('click').click(function () {                   
                    _self.properties.contentObj.hide_load_block();
                    return false;
                });
                $('#' + _self.properties.btnSaveID).on('click', function () {
...
if (typeof exports === 'object') {
    ...
    exports.fieldEditorSelect = fieldEditorSelect;
}
