...
class AdminSeoAdvanced extends \Controller
{
    ...
    public function edit($module_gid, $method)
    {
        ...
        if (empty($default_settings['url_vars'])) {
            $default_settings['url_vars'] = [];
        }
        if (empty($default_settings['url_postfix'])) {
            $default_settings['url_postfix'] = [];
        }
        ...
        if ($this->input->post('btn_save')) {
            ...
            if (!empty($url_template_data)) {
                $index = 0;
                $url_template_data_default = [];
                $url_template_data_default[] = ['type' => 'text', 'value' => $module_gid . '/' . $method . (!empty($default_settings["url_vars"]) && !empty($default_settings["url_postfix"]) ? '/' : ''), 'var_num' => '', 'var_name' => '', 'var_type' => '', 'var_default' => ''];
                $url_vars_data = array_values($default_settings["url_vars"]);
                foreach ($url_vars_data as $value) {
                    if ($index) {
                        $url_template_data_default[] = ['type' => 'text', 'value' => '/', 'var_num' => '', 'var_name' => '', 'var_type' => '', 'var_default' => ''];
                    }
                    $url_template_data_default[] = ['type' => 'tpl', 'value' => '', 'var_num' => ++$index, 'var_name' => key($value), 'var_type' => current($value), 'var_default' => 'empty'];
                }
                $url_postfix_data = array_values($default_settings["url_postfix"]);
                foreach ($url_postfix_data as $value) {
                    if ($index) {
                        $user_settings["url_template_data"][] = ['type' => 'text', 'value' => '/'];
                    }
                    $url_template_data_default[] = ['type' => 'postfix', 'value' => '', 'var_num' => ++$index, 'var_name' => key($value), 'var_type' => current($value), 'var_default' => ''];
                }
        ...
        if (!empty($user_settings["url_template"])) {
            ...
            $index = 0;
            $user_settings["url_template_data"] = [];
            $user_settings["url_template_data"][] = ['type' => 'text', 'var_num' => '', 'value' => $module_gid . '/' . $method . (!empty($default_settings["url_vars"]) || !empty($default_settings["url_postfix"]) ? '/' : '')];
            $url_vars_data = $default_settings["url_vars"];
            foreach ($url_vars_data as $value) {
                if ($index) {
                    $user_settings["url_template_data"][] = ['type' => 'text', 'var_num' => '', 'value' => '/'];
                }
                $part = ['type' => 'tpl', 'var_num' => ++$index, 'var_name' => key($value), 'var_type' => current($value), 'var_default' => 'empty'];
            ...
            foreach ($url_postfix_data as $value) {
                ...
                $part = ['type' => 'postfix', 'var_num' => ++$index, 'var_name' => key($value), 'var_type' => current($value), 'var_default' => ''];
                $user_settings["url_template_data"][] = $part;
            }
        }

        $user_settings["url_template_options"] = [];
