...
class WallEvents extends \Controller
{
    ...
    private function getEvents($data, $use_load = false, $items_on_page = null, $page = null)
    {
        ...
        $wall_events_types = $this->Wall_events_types_model->getWallEventsTypesGids($types_params);
        $user_feeds = $this->Wall_events_permissions_model->getUserFeeds($user_id);
        $show_wall_events_types = [];

        $is_friendlist_installed = $this->pg_module->is_module_installed('friendlist');
        $is_favorites_installed = $this->pg_module->is_module_installed('favorites');
        if ($is_friendlist_installed) {
            $this->load->model('Friendlist_model');
        }
        if ($is_favorites_installed) {
        ...
        switch ($data['place']) {
            ...
            default:
                ...
                if ($is_favorites_installed) {
                    $favorites_ids = $this->Favorites_model->get_list_users_ids($user_id);
                }
                if ($friends_ids || $favorites_ids) {
                    $friends_ids = array_unique(array_merge($friends_ids, $favorites_ids));
                    if ($is_wall_owner) {
        ...
        if ($result['count_all']) {
            ...
            if ($wall_events) {
                ...
                $wall_events_users_ids = $wall_events_users = [];
                ...
                if ($wall_events_users_ids) {
                    ...
                    $wall_events_users = $this->Users_model->get_users_list_by_key(null, null, null, [], $wall_events_users_ids);
                    foreach ($wall_events_users as $e_user) {
                        $result['users'][$e_user['id']] = [
                            'output_name' => $e_user['output_name'],
                            'media'       => ['user_logo' => $e_user['media']['user_logo']],
                        ];
    ...
    public function ajaxPost()
    {
        ...
        $required_fields = (!$data['text'] && $data['embed_code']) ? ['embed_data'] : ['text'];
    ...
    {
        $user_id = (int)$this->session->userdata('user_id');
        $id_wall = in_array($place, ['homepage', 'myprofile']) ? $user_id : (int)$id_wall;
    ...
    private function post($id_wall, $data = [], $id = 0, $required_fields = [])
    {
        $user_id = $this->session->userdata('user_id');
