...
class AdminMailList extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['Menu_model', 'Users_model', 'Mail_list_model']);
    ...
    public function users($filter = 'all', $page = 1, $user_type = 0)
    {
        ...
        // Set default filter
        if (!in_array($filter, ['all', 'subscribed', 'not_subscribed'])) {
        ...
        if ($this->input->post('btn_cancel', true)) {
            ...
            $fields = ['email', 'name', 'date', 'user_type',
    ...
    public function ajaxDeleteFilter()
    {
        ...
        exit($this->Mail_list_model->deleteFilter($id_filter));
    }
