function media(optionArr)
{
    ...
    this.init_links = function () {
        $("[data-media=" + _self.properties.addPhotoID + "]").unbind('click').on('click', function () {
            _self.display_upload_photos_form();
        });
        $("[data-media=" + _self.properties.addVideoID + "]").unbind('click').on('click', function () {
            _self.display_upload_video_form();
        });
        $("[data-media=" + _self.properties.addAudioID + "]").unbind('click').on('click', function () {
    ...
    this.setLoadOnScroll = function () {
        if (_self.properties.load_on_scroll) {
            $(window).scrollTop(0);
            $(window).on('scroll.media', function () {
