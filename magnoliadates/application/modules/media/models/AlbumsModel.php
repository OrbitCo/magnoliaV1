...
class AlbumsModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        $this->ci->cache->registerService(ALBUM_TABLE);
    }
     ...
     * @return array
     */
    public function getAlbumsList($params = [], $filter_object_ids = null, $order_by = null, $page = null, $items_on_page = null, $formatted = true, $lang_id = false)
    {
    ...
    public function getAlbumById($album_id, $lang_id = false, $languages = [])
    {
        $select_attrs = $this->fields;
        if ($lang_id) {
            foreach ($select_attrs as $key => $value) {
        ...
        if ($lang_id || !empty($languages)) {
            $result = $this->ci->db->select(implode(", ", $select_attrs))->from(ALBUM_TABLE)->where("id", $album_id)->get()->result_array();
        } else {
            $result =  $this->ci->cache->get(ALBUM_TABLE, $album_id, function () use ($select_attrs, $album_id) {
                $result = $this->ci->db->select(implode(", ", $select_attrs))->from(ALBUM_TABLE)->where("id", $album_id)->get()->result_array();
        ...
        $result = $this->format_albums([0 => $result[0]]);
    ...
    public function formatAlbums($data)
    {
        ...
        if ($media_ids) {
            $medias_result = $this->ci->Media_model->get_media(null, null, null, [], $media_ids);
    ...
    public function save($album_id = null, $attrs = [], $langs = [])
    {
        ...
        $this->ci->cache->delete(ALBUM_TABLE, $album_id);
    ...
    public function getAlbumsCount($params = [], $filter_object_ids = null)
    {
        $this->ci->db->select("COUNT(*) AS cnt");
        $this->ci->db->from(ALBUM_TABLE);
        if (!isset($params['where']['is_default'])) {
    ...
    public function deleteUserAlbums($id_user)
    {
        ...
        foreach ($albums as $album) {
            $albums_ids[] = $album['id'];
            $this->ci->cache->delete(ALBUM_TABLE, $album);
        }
        if ($albums_ids) {
    ...
    public function langDedicateModuleCallbackDelete($lang_id = false)
    {
        if ($lang_id !== false) {
            $this->ci->load->dbforge();
            $fields_exists = $this->ci->db->list_fields(ALBUM_TABLE);
