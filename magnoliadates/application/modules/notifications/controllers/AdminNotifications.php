...
class AdminNotifications extends \Controller
{
    ...
    public function index($order = "date_add", $order_direction = "DESC", $page = 1)
    {
        ...
        $attrs = [];
        $current_settings = isset($_SESSION["nf_list"]) ? $_SESSION["nf_list"] : [];
        ...
        if ($notif_count > 0) {
            $notifications = $this->Notifications_model->get_notifications_list($page, $items_on_page, [$order => $order_direction], $attrs);
    ...
    public function settings()
    {
        ...
        if ($this->input->post('btn_save')) {
            $post_data = [
                'mail_charset'    => trim($this->input->post('mail_charset', true)),
                'mail_protocol'   => trim($this->input->post('mail_protocol', true)),
                'mail_mailpath'   => trim($this->input->post('mail_mailpath', true)),
                'mail_smtp_host'  => trim($this->input->post('mail_smtp_host', true)),
                'mail_smtp_user'  => trim($this->input->post('mail_smtp_user', true)),
                'mail_smtp_pass'  => trim(htmlspecialchars($this->input->post('mail_smtp_pass', true))),
                'mail_smtp_port'  => trim($this->input->post('mail_smtp_port', true)),
                'mail_useragent'  => trim($this->input->post('mail_useragent', true)),
                'mail_from_email' => trim($this->input->post('mail_from_email', true)),
                'mail_from_name'  => trim($this->input->post('mail_from_name', true)),
            ];
    ...
    public function templates($filter = "all", $order = "date_add", $order_direction = "DESC", $page = 1)
    {
        ...
        $attrs = [];
        $current_settings = isset($_SESSION["nf_templates_list"]) ? $_SESSION["nf_templates_list"] : [];
        ...
        $filter_data = [
            ...
            "text" => $this->Templates_model->get_templates_count(["where" => ["content_type" => "text"]]),
            "html" => $this->Templates_model->get_templates_count(["where" => ["content_type" => "html"]]),
        ];

        switch ($filter) {
            case 'text':
                $attrs["where"]['content_type'] = "text";
                break;
            case 'html':
                $attrs["where"]['content_type'] = "html";
                break;
            default:
        ...
        if ($templates_count > 0) {
            $templates = $this->Templates_model->get_templates_list($page, $items_on_page, [$order => $order_direction], $attrs);
    ...
    public function templateEdit($id_template = null)
    {
        ...
        if ($data["content_type"] == "html") {
            ...
            foreach ($languages as $lang_id => $lang) {
                $content = isset($data_content[$lang_id]) ? $data_content[$lang_id] : [];
    ...
    private function formatPoolTable($order = "id", $order_direction = "DESC", $page = 1)
    {
        ...
        $attrs = [];
        $current_settings = isset($_SESSION["senders_list"]) ? $_SESSION["senders_list"] : [];
        if (!isset($current_settings["order"])) {
        ...
        if ($notif_count > 0) {
            $senders = $this->Sender_model->get_senders_list($page, $items_on_page, [$order => $order_direction], $attrs);
    ...
    public function poolDelete($id = null)
    {
