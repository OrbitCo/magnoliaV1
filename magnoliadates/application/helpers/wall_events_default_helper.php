...
if (!function_exists('add_wall_event')) {
    function add_wall_event($event_type_gid, $id_wall, $id_poster, $data, $id_object = 0, $file_name = '', $required_fields = [])
    {
        $result['errors'] = [];
        $CI = &get_instance();
        if ($CI->pg_module->is_module_installed('wall_events')) {
            ...
            $wall_perm = $CI->Wall_events_permissions_model->getUserPermissions($id_wall);

            $is_post_allowed = false;

            if ($event_type_gid == 'wall_post') {
                if ($id_wall == $id_poster || !empty($wall_perm[$event_type_gid]['post_allow'])) {
                    $is_post_allowed = true;
                }
            } else {
                if ($id_wall == $id_poster || !empty($wall_perm[$event_type_gid]['feed'])) {
                    $is_post_allowed = true;
                }
            }
            ...
            if ($is_post_allowed) {
                $permissions = $wall_perm[$event_type_gid]['permissions'] ?? 0;
                $event_permissions = $wall_perm[$event_type_gid]['permissions'] ?? 3;
