...
Color.prototype.getHex = function (webColors,CBMode,varNr) {
    ...
    var v = V * 255;
    var k = (max > 0) ? v / max : 0;
    r = Math.round(v - (v - this.baseR * k) * S); if (r > 255) {
        r = 255;
    }
    g = Math.round(v - (v - this.baseG * k) * S); if (g > 255) {
        g = 255;
    }
    b = Math.round(v - (v - this.baseB * k) * S); if (b > 255) {
        b = 255;
    }
    if (webColors) {
...
function createScheme(setDefaults)
{
    ...
    if (setDefaults) {
        for (var i = 0; i < 8; i++) {
            col[i].setVariantPreset(usedPreset);
        }
    }
    drawSample();
}
...
function getAllSetsExample()
{
    ...
    for (var i = 0; i < 8; i++) {
        col[0].setVariantPreset(usedPreset);
    }
    return vari;
}
...
function drawVar(colNr,varNr)
{
    ...
    for (var i = 20; i >= 0; i--) {
        ...
        for (var j = 20; j >= 0; j--) {
            ...
            if (c.S[0] == S && c.V[0] == V) {
                buff += 'background: red url(\'ring-w.gif\') center no-repeat; ';
            }
...
function Init()
{
    ...
    $('#sample_preset').on('change', function () {
        switchPreset($(this).val());
    });
    $('#sample_scheme_type').on('change', function () {
        switchSchemeType($(this).val());
    });
//  $('#manual_color').on('blur', function(){
//      searchRGB($(this).val());
//  });
    $('#manual_h').on('keyup', function () {
...
function writePgSample(ret)
{
    ...
    for (m in schemeSettings) {
        if (schemeSettings[m].type != 'color') {
            continue;
        }
