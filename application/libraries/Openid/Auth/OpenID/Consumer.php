...
class Auth_OpenID_Consumer
{
     ...
     */
    public function getDiscoveryObject(
        $session,
        $openid_url,
        $session_key_prefix
    ) {
        return new Auth_Yadis_Discovery($session,
            $openid_url,
                                        $session_key_prefix);
    }
    ...
    public function complete($current_url, $query = null)
    {
        ...
        $response = $this->consumer->complete($message,
            $endpoint,
                                              $current_url);
        $this->session->del($this->_token_key);

        if (in_array($response->status, [Auth_OpenID_SUCCESS,
                                              Auth_OpenID_CANCEL, ])) {
...
class Auth_OpenID_GenericConsumer
{
    ...
    {
        $mode = $message->getArg(Auth_OpenID_OPENID_NS,
            'mode',
                                 '<no mode set>');
        ...
        $method = Auth_OpenID::arrayGet($mode_methods,
            $mode,
                                        '_completeInvalid');
    ...
    public function _discoverAndVerify($claimed_id, $to_match_endpoints)
    {
        ...
        return $this->_verifyDiscoveryServices($claimed_id,
            $services,
                                               $to_match_endpoints);
    }
    ...
    /**
     */
    public function _verifyDiscoveryServices(
        $claimed_id,
        $services,
        $to_match_endpoints
    ) {
    ...
    public function _createCheckAuthRequest($message)
    {
        ...
        $ca_message->setArg(Auth_OpenID_OPENID_NS,
            'mode',
    ...
    {
        $is_valid = $response->getArg(Auth_OpenID_OPENID_NS,
            'is_valid',
                                      'false');
...
class Auth_OpenID_AuthRequest
{
    ...
    public function getMessage($realm, $return_to = null, $immediate = false)
    {
        ...
        if (!$this->_anonymous) {
            ...
            $message->setArg(Auth_OpenID_OPENID_NS,
                'identity',
                             $request_identity);

            if ($message->isOpenID2()) {
                $message->setArg(Auth_OpenID_OPENID2_NS,
                    'claimed_id',
                                 $claimed_id);
            }
        }
