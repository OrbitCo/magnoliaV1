...
class phRETS
{
    ...
    public function SearchQuery($resource, $class, $query = "", $optional_params = [])
    {
        $this->reset_error_info();
    ...
    public function Search($resource, $class, $query = "", $optional_params = [])
    {
        $data_table = [];
        ...
        if ($xml->METADATA && $xml->METADATA->{'METADATA-LOOKUP_TYPE'}) {
            foreach ($xml->METADATA->{'METADATA-LOOKUP_TYPE'} as $key) {
                if (!empty($key->attributes()->Lookup)) {
                    ...
                    $this_table[] = ['Lookup' => "{$key->attributes()->Lookup}", 'Values' => $this_lookup];
                }
            }
        }
    ...
    public function GetMetadataResources($id = 0)
    {
        ...
        if ($xml->METADATA) {
            foreach ($xml->METADATA->{'METADATA-RESOURCE'}->Resource as $key => $value) {
                $this_resource["{$value->ResourceID}"] = [
    ...
    public function GetMetadataTypes($id = 0)
    {
        ...
        if ($xml->METADATA) {
            foreach ($xml->METADATA->{'METADATA-CLASS'} as $key => $value) {
                ...
                $return_data[] = ['Resource' => "{$resource}", 'Data' => $this_resource];
            }
        }
    ...
    public function Connect($login_url, $username, $password, $ua_pwd = "")
    {
        ...
        // start cURL magic
        $this->ch = curl_init();
        curl_setopt($this->ch, CURLOPT_HEADERFUNCTION, [&$this, 'read_custom_curl_headers']);
        if ($this->debug_mode == true) {
    ...
    public function RETSRequest($action, $parameters = "")
    {
        ...
        curl_setopt($this->ch, CURLOPT_HTTPHEADER, [trim($request_headers)]);
        // do it
        ...
        return [$this->last_response_headers_raw, $response_body];
    }
