...
function uploader(optionArr)
{
    ...
    this.Init = function (options) {
      ...
      _self.objects.fileInput.unbind('change').on('change', function (event) {
      ...
      if (typeof _self.objects.dropZone !== 'undefined') {

          _self.objects.dropZone.unbind('dragenter').on('dragenter', function () {
            _self.objects.dropZone.addClass('hover');
            return false;
          });

          _self.objects.dropZone.unbind('dragover').on('dragover', function () {
            return false;
          });
          ...
          _self.objects.dropZone.unbind('dragleave').on('dragleave', function (e) {
            var is_leave = false;
            if (e.relatedTarget) {
          ...
          _self.objects.dropZone.unbind('drop').on('drop', function (e) {
        ...
        if (_self.properties.sendType === 'file') {
            $('#' + _self.properties.sendId).unbind('click').on('click', function () {
    ...
    this.process = function (file) {
        if (file.size > _self.properties.maxFileSize && _self.properties.maxFileSize > 0) {
            if (_self.properties.cbOnProcessError instanceof Function) {
                ...
                _self.properties.errorObj.show_error_block(this.getMessageLang('exceeded'), 'error');
                _self.resetFileInput();
                this.addMessage(this.getMessageLang('exceeded'), 'error');
            }
            _self.objects.dropZone.removeClass('set-uploader-form');
    ...
    this.displayFile = function (file) {
        ...
        thumb.find('[data-action="delete"]').on('click', function () {
...
if (typeof exports === 'object') {
    ...
    exports.uploadErrorObject = uploadErrorObject;
    exports.uploaderObject = uploaderObject;
