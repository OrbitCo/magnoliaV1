...
class AdminCountries extends \Controller
{
    ...
    {
        if ($sort_mode == 0) {
            $order_for_not_sorted  = ["priority" => "ASC", "lang_" . $this->pg_language->current_lang_id => "ASC"];
            $params_for_not_sorted = [];
        } else {
            $order_for_not_sorted = ["lang_" . $this->pg_language->current_lang_id => "ASC"];
            $order_for_sorted     = ["priority" => "ASC", "lang_" . $this->pg_language->current_lang_id => "ASC"];

            $params_for_sorted["where"]["sorted !="]    = 0;
            $params_for_not_sorted["where"]["sorted ="] = 0;

            $sorted_countries_list = $this->Countries_model->get_countries($order_for_sorted, $params_for_sorted, [], $this->pg_language->current_lang_id);

            $this->view->assign('sorted', $sorted_countries_list);
        }
        $installed_countries_list = $this->Countries_model->get_countries($order_for_not_sorted, $params_for_not_sorted, [], $this->pg_language->current_lang_id);
    ...
    public function country($country_code, $sort_mode = 0)
    {
        ...
        if ($sort_mode == 0) {
            ...
            $sorted_regions_list = $this->Countries_model->get_regions($country_code, $order_for_sorted, $params_for_sorted, [], $this->pg_language->current_lang_id);

            $this->view->assign('sorted', $sorted_regions_list);
        }

        $installed_regions_list = $this->Countries_model->get_regions($country_code, $order_for_not_sorted, $params_for_not_sorted, [], $this->pg_language->current_lang_id);
    ...
    public function region($country_code, $id_region, $page = 1, $sort_mode = 0)
    {
        ...
        $current_settings = isset($_SESSION["cities_list"]) ? $_SESSION["cities_list"] : [];
        ...
        if ($sort_mode == 0) {
            ...
            $sorted_cities_list = $this->Countries_model->get_cities($page, $items_on_page, $order_for_sorted, $params_for_sorted, [], $this->pg_language->current_lang_id);

            $this->view->assign('sorted', $sorted_cities_list);
        }

        if ($cities_count) {
            $cities_list = $this->Countries_model->get_cities($page, $items_on_page, $order_for_not_sorted, $params, [], $this->pg_language->current_lang_id);
    ...
    public function install($step = 'country', $country_code = '')
    {
        ...
        switch ($step) {
            case 'country': {
                    ...
                    $installed_countries_list = $this->Countries_model->get_countries(["name" => "ASC"]);
            ...
            case 'region': {
                ...
                if (!empty($country)) {
                    ...
                    $installed_regions_list = $this->Countries_model->get_regions_by_code($country_code, ["code" => "ASC"]);
                    $this->view->assign('country', $country);
