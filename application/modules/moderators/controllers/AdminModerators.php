...
class AdminModerators extends \Controller
{
    ...
    public function index($order = null, $order_direction = null, $page = null)
    {
        ...
        $filter_data = [
            ...
            "not_active" => $this->Moderators_model->get_users_count(["where" => ["status" => 0]]),
            "active" => $this->Moderators_model->get_users_count(["where" => ["status" => 1]]),
            "admin" => $this->Moderators_model->get_users_count(["where" => ["user_type" => 'admin']]),
            "moderator" => $this->Moderators_model->get_users_count(["where" => ["user_type" => 'moderator']]),
        ];
        ...
        if ($users_count > 0) {
            $users = $this->Moderators_model->get_users_list($page,
                $items_on_page,
    ...
    public function edit($user_id = null)
    {
        ...
        $methods = $this->Moderators_model->getMethodsByGroups(['where' => ['is_hidden' => 0]]);
        if (!empty($data["permission_data"])) {
            ...
            if (!$user_id && empty($validate_data["errors"])) {
                 ...
                 */
                $methods_default = $this->Moderators_model->getMethods(['where' => ['module' => 'moderation']]);
                if (empty($methods_default)) {
                    //get the first method from the list
                    $methods_default = $this->Moderators_model->getMethods(['limit' => 1]);
                }
                foreach ($methods as $module_group => $module_data) {
                    if (!empty($module_data["methods"])) {
                        foreach ($module_data["methods"] as $key => $method) {
