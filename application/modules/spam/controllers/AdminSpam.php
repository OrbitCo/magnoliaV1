...
class AdminSpam extends \Controller
{
    ...
    public function index($filter = "", $order = null, $order_direction = null, $page = 1)
    {
        ...
        $current_settings = isset($_SESSION["spam_alerts_list"]) ? $_SESSION["spam_alerts_list"] : [];
        ...
        if ($alerts_count > 0) {
            $this->Spam_alert_model->set_format_settings(["get_reason" => true]);
            if ($order === "poster") {
                $order = "id_poster";
            }
            $alerts = $this->Spam_alert_model->get_alerts_list($page, $items_on_page, [$order => $order_direction], $filter);
    ...
    public function alertsShow($id)
    {
        ...
        $this->Spam_alert_model->set_format_settings(["get_link" => true, "get_object" => true, "get_reason" => true, "get_deletelink" => true, "get_subpost" => true]);
