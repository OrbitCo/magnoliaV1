...
class MediaModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        $this->ci->cache->registerService(MEDIA_TABLE);
    ...
    {
        $result = $this->ci->cache->get(MEDIA_TABLE, $media_id, function () use ($media_id) {

            $ci = &get_instance();

            $result = $ci->db->select(implode(", ", $this->fields))->from(MEDIA_TABLE)->where("id", $media_id)->get()->result_array();
        ...
        if (empty($result)) {
            ...
            if ($formatted) {
                $result = $this->format_items([0 => $result[0]], $format_owner, $format_user);
            }
    ...
    public function validateVideo($data, $video_name = '')
    {
        ...
        if (!empty($data["embed_code"])) {
            ...
            if ($embed_data !== false) {
                ...
                $return["data"]["media_video_data"] = serialize(['data' => $embed_data]);
    ...
    public function saveImage($id, $data, $file_name = "", $moderation = false, $indicator = true)
    {
        ...
        if (!empty($id)) {
            ...
            $this->ci->cache->delete(MEDIA_TABLE, $id);
        } else {
        ...
        if (!empty($file_name) && !empty($id)) {
            ...
            $img_return = $this->ci->Uploads_model->{$upload_method}($this->file_config_gid,
                $data['id_owner'],
                $file_name);
    ...
    public function updateChildren($id_parent, $data)
    {
        ...
        $result = false;
        $media_ids = $this->get_media_ids(null, null, null, ['where' => ['id_parent' => $id_parent]]);
        if ($media_ids) {
    ...
    public function updateChildrenPermissions($id_media, $permissions = null)
    {
        ...
        $result = false;
        $media_ids = $this->get_media_ids(null, null, null, ['where' => ['id_parent' => $id_media]]);
        if ($media_ids) {
    ...
    public function saveVideo($id, $data, $video_name = "", $create_event = false)
    {
        ...
        if (!empty($video_name) && !empty($id) && isset($_FILES[$video_name]) && is_array($_FILES[$video_name]) && is_uploaded_file($_FILES[$video_name]["tmp_name"])) {
            $this->ci->load->model("Video_uploads_model");
            $video_return = $this->ci->Video_uploads_model->upload($this->video_config_gid,
                $data['id_owner'],
                $video_name,
                $id,
                $data,
    ...
    public function saveAudio($id, $data, $file_name = "", $create_event = false, $indicator = true)
    {
        ...
        if (!empty($file_name) && !empty($id)) {
            $this->ci->load->model("Audio_uploads_model");
            $audio_return = $this->ci->Audio_uploads_model->upload($this->audio_config_gid,
                $data['id_owner'],
                $file_name,
                $id,
                $data,
     ...
     * @return array
     */
    public function getMedia($page = 1, $items_on_page = 20, $order_by = null, $params = [], $filter_object_ids = null, $format_items
    = true, $format_owner = false, $format_user = false, $safe_format = false): array
    {
        ...
        if ($format_items) {
            $result = $this->formatItems($result, $format_owner, $format_user, $safe_format);
        }

        return $result;
    }

    public function getMediaByKey($page = 1, $items_on_page = 20, $order_by = null, $params = [], $filter_object_ids
    = null, $format_items = true, $format_owner = false, $format_user = false, $safe_format = false): array
    {
        $media = $this->getMedia($page,
            $items_on_page,
            $order_by,
            $params,
            $filter_object_ids,
     ...
     * @return array
     */
    public function getMediaIds($page = 1, $items_on_page = 20, $order_by = null, $params = [], $filter_object_ids = null)
    {
        $objects = [];
    ...
    public function formatItems($data, $format_owner = false, $format_user = false, $safe_format = false): array
    {
        ...
        foreach ($data as $key => &$item) {
            if ($item['upload_gid'] == $this->file_config_gid) {
                if (!empty($item["mediafile"]) && $item['permissions'] > self::PERM_OMIT) {
                    $item["media"]["mediafile"] = $this->ci->Uploads_model->formatUpload($item['upload_gid'],
                        $item['id_owner'],
                        $item['mediafile']);
                } else {
                    $item["media"]["mediafile"] = $this->ci->Uploads_model->formatDefaultUpload($item['upload_gid']);
                }
                $item["media"]["mediafile"]['photo_title'] = str_replace('[id]',
                    $item['id'],
                    l('text_media_photo', 'media'));
                $item["media"]["mediafile"]['photo_alt'] = str_replace('[id]',
                    $item['id'],
                    l('text_media_photo', 'media'));
                ...
                if (!empty($item["media_video"])) {
                    ...
                    $item["video_content"] = $this->ci->Video_uploads_model->formatUpload($this->video_config_gid,
                        $item['id_owner'],
                        $media_video,
                        $item['media_video_image'],
                        $item['media_video_data']['data']['upload_type'],
                        $item["media_video_data"]['data']['isHTML5']);
                }
            } elseif ($item['upload_gid'] == $this->audio_config_gid) {
                if ($this->ci->pg_module->is_module_active('audio_uploads')) {
                    $item["media"]["mediafile"] = $this->ci->Audio_uploads_model->formatUpload($this->audio_config_gid,
                        $item['id_owner'],
                        $item['mediafile'],
                        '',
                        false,
            ...
            $item["settings"] = $item["settings"] ? (array)unserialize($item["settings"]) : [];
        ...
        if ($users_ids) {
            $users = $this->Users_model->get_users_list_by_key(null, null, null, [], $users_ids, false);
    ...
    public function getMediaCount($params = [], $filter_object_ids = null)
    {
        if ($this->ci->session->userdata('auth_type') == 'user') {
            $user_id = $this->ci->session->userdata('user_id');
            $this->ci->load->model('Blacklist_model');
    ...
    public function videoCallback($id, $status, $data, $errors)
    {
        ...
        if ($status == 'start') {
            $update["media_video_data"] = [];
        }

        if (!isset($update["media_video_data"]["data"])) {
            $update["media_video_data"]["data"] = [];
        }
    ...
    public function deleteMedia($media_id)
    {
        ...
        $this->ci->cache->delete(MEDIA_TABLE, $media_id);
        if (!$media) {
            return;
        }
        ...
        if (!empty($media['mediafile'])) {
            $this->update_children($media['id'], ['mediafile' => '']);
        ...
        if (!empty($media['media_video'])) {
            $this->update_children($media['id'], ['media_video' => '']);
            if ($media['id_user'] == $media['id_owner']) {
                if ($media['media_video'] !== 'embed' && $media['upload_gid'] != 'gallery_audio') {
                    $this->ci->db->where('media_video', $media['media_video']);
                    $this->ci->db->delete(MEDIA_TABLE);
                    $this->ci->load->model('Video_uploads_model');
                    $this->ci->Video_uploads_model->delete_upload($this->video_config_gid,
                        $media['id_owner'],
                        $media['media_video'],
                        $media['media_video_image']);
            ...
            $this->ci->load->model('Audio_uploads_model');
            $this->ci->Audio_uploads_model->delete_upload($this->audio_config_gid,
                $media['id_owner'],
                $media['mediafile']);
        }
        ...
        if (!empty($children_ids) && is_array($children_ids)) {
            foreach ($children_ids as $children_id) {
                ...
                $this->ci->cache->delete(MEDIA_TABLE, $children_id);
            }
        }
    ...
    public function getList($user_id, $param = 'all', $page = 1, $album_id = 0, $use_permissions = true, $order_by = ['date_add' => 'DESC'])
    {
        ...
        switch ($param) {
            case 'all':
                ...
                break;
            case 'photo':
                $where['where']['upload_gid'] = $this->file_config_gid;
                break;
            case 'video':
                $where['where']['upload_gid'] = $this->video_config_gid;
                break;
            case 'audio':
                $where['where']['upload_gid'] = $this->audio_config_gid;
                break;
            case 'favorites':
        ...
        $media = (!$album_id || $media_ids) ? $this->get_media($exists_page,
            $items_on_page,
            $order_by,
            $where,
            $media_ids) : [];
    ...
    = [])
    {
        ...
        switch ($param) {
            case 'all':
                $params['where_in']['upload_gid'] = ['gallery_image', 'gallery_video', 'gallery_audio'];
                break;
            case 'photo':
                $params['where']['upload_gid'] = $this->file_config_gid;
                break;
            case 'video':
                $params['where']['upload_gid'] = $this->video_config_gid;
                break;
            case 'audio':
                $params['where']['upload_gid'] = $this->audio_config_gid;
                break;
            case 'favorites':
        ...
        $return['media'] = (!$album_id || $media_ids) ? $this->get_media(null,
            null,
            $order_by,
            $params,
            $media_ids,
    ...
    public function getAlbums($id_user, $page = 1)
    {
        ...
        $albums = $this->ci->Albums_model->get_albums_list($params,
            null,
            null,
            $exists_page,
            $items_on_page,
    ...
    = ['date_add' => 'DESC'], $filter_duplicate = false)
    {
        ...
        $media_ids = [$return['next'], $return['previous']];
        $medias = $this->get_media_by_key(null, null, null, [], $media_ids);
    ...
    public function isAccessPermitted($media_id, $media = [])
    {
        if (!$media) {
            $media = $this->get_media_by_id($media_id);
        }
    ...
    public function formatWallEvents($events, $use_spam = true): array
    {
        ...
        foreach ($events as $e) {
            ...
            foreach ($e['data'] as $media) {
                if (($this->getMediaCount(['where_in' => ['id' => $media['id']]])) != 0) {
        ...
        if ($media_ids) {
            $params = ['where_in' => [
                'upload_gid' => [$this->file_config_gid, $this->video_config_gid, $this->audio_config_gid]
            ]];
            $medias = $this->getMediaByKey(null, null, null, $params, $media_ids);
            if ($medias) {
                $friends_ids = [];
                if ($this->user_id) {
                    if ($this->ci->pg_module->is_module_installed('friendlist')) {
                    ...
                    if (is_array($e['data'])) {
                        foreach ($e['data'] as $mkey => $media) {
                            if (!empty($medias[$media['id']]) && $is_permissions_allowed($medias[$media['id']],
                                    $friends_ids,
                                    $this->user_id,
                                    $this->ci->session->userdata('show_adult'))) {
    ...
    protected function createWallEvent($media_id)
    {
        $media = $this->get_media_by_id($media_id);
        if ($media && $media['id_owner'] == $media['id_user']) {
            ...
            return add_wall_event($e_gid, $media['id_user'], $media['id_user'], $event_data, $media['id']);
        }
    ...
    public function getSeoSettings($method = '', $lang_id = '')
    {
        if (!empty($method)) {
            ...
            $actions = ['index', 'all', 'photo', 'video', 'albums', 'audio'];
            $return = [];
        ...
        switch ($method) {
            case 'index':
                return [
                    ...
                    'url_vars' => [],
                    'url_postfix' => [
                        'section-code' => ['section-code' => 'literal', 'section-name' => 'literal'],
                    ],
                    'optional' => [],
                ];
                break;
    ...
    public function getSitemapXmlUrls($generate = true)
    {
        ...
        $return = [];
        $types = ['all' => 0.5, 'photo' => 0.5, 'video' => 0.5, 'albums' => 0.5, 'audio' => 0.5];
            ...
            if ($generate === true) {
                ...
                foreach ($languages as $lang_id => $lang_data) {
                    if ($this->ci->pg_language->is_active($lang_id) === true) {
                        ...
                        $return[] = [
                            "url" => rewrite_link('media', $type, [], false, $lang_code, $lang_canonical),
                ...
                $return[] = [
                    "url" => rewrite_link('media', $type, [], false, null, $lang_canonical),
                    "priority" => $user_settings['priority'],
                    "page" => $type,
                ];
            }
    ...
    public function spamCallback($action, $data)
    {
        switch ($action) {
            ...
            case 'get_content':
                ...
                $return = [];
                foreach ($data as $id) {
                    if (($this->get_media_count(['where_in' => ['id' => $id]])) == 0) {
                    ...
                    if ($media["upload_gid"] == "gallery_image") {
                        ...
                        if (preg_match_all("/id=\"default_player_([0-9]+)\">/is", $event['html'], $match)) {
                            ...
                            foreach ($match[1] as $key => $rand) {
                                $media['video_content']['embed'] = str_replace($rand,
                                    $rand . "_" . $key,
    ...
    {
        return $this->get_gallery_list($count, $param, null, null, $order_by = ['date_add' => 'DESC'], $attrs);
    }
    ...
    protected function deleteMediaByUserId($id_user, $type)
    {
        if ($type === 'gallery') {
    ...
    public function installRatingsFields($fields = [])
    {
        ...
        foreach ((array)$fields as $field_name => $field_data) {
            if (!in_array($field_name, $table_fields)) {
                $this->ci->dbforge->add_column(MEDIA_TABLE, [$field_name => $field_data]);
            }
        }
    }
    ...
    public function sendEventUpload($type = null, $user_id = null, $media_id = null)
    {
        if ($user_id && $type) {
            ...
            $event_handler->dispatch($event, 'media_upload_' . $type);
        }
    }
    ...
    public function rotation($photo_id, $rotate)
    {
        ...
        if ($media && $media['id_owner'] == $this->user_id) {
            $this->ci->load->model('Uploads_model');
            $this->ci->Uploads_model->rotateUpload($this->file_config_gid,
                $media['id_owner'],
                $media['mediafile'],
                $rotate);
