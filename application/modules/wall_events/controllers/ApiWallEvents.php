...
class ApiWallEvents extends \Controller
{
    ...
    private function getEventsInternal($data, $use_load = false, $items_on_page = null, $page = null)
    {
        ...
        switch ($data['place']) {
            ...
            default:
                ...
                if ($is_favorites_installed) {
                    $favorites_ids = $this->Favorites_model->get_list_users_ids($this->user_id);
                }
                if ($friends_ids || $favorites_ids) {
                    $friends_ids = array_merge($friends_ids, $favorites_ids);
                    if ($is_wall_owner) {
        ...
        if ($result['count_all']) {
            ...
            if ($wall_events) {
                ...
                if ($wall_events_users_ids) {
                    ...
                    $wall_events_users = $this->Users_model->get_users_list_by_key(null, null, null, [], $wall_events_users_ids, true, true);
    ...
    */
    public function post()
    {
        ...
        $result = $this->postInternal($id_wall, $text, 0, ['text']);
    ...
    */
    ...
    private function postInternal($id_wall, $text = '', $id = 0, $required_fields = [])
    {
        $data['text'] = $text;
