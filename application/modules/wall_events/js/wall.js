function Wall()
{
    ...
    this.bind_delete = function() {
        $('.delete_wall_event, .js-delete_wall_event').off('click').on('click', function(e) {
            e.preventDefault();
            var id = $(this).data('id');
            var text = $(this).data('message');
            _self.viewAlert(text, 'deletePost', id);
        });
    };
        ...
        $.ajax({
            ...
            success: function (resp, textStatus, jqXHR) {
                ...
                if (resp.msg) {
                    ...
                    _self.properties.errorObj.show_error_block(resp.msg, status);
                }
                if (typeof onsuccess == 'function') {
                    onsuccess();
                }
