...
class BannersStatModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        $this->ci->cache->registerService(TABLE_BANNERS_STAT_TEMP);
        //TODO (nsavanaev) cache
        $this->ci->cache->registerService(TABLE_BANNERS_STAT_GENERAL);
        $this->ci->cache->registerService(TABLE_BANNERS_STAT_HOURLY);
    }
        ...
        if ($banner_id && $action) {
            switch ($this->get_stat_type) {
                ...
                case 'db':
                    ...
                    $stat = $this->ci->cache->get(TABLE_BANNERS_STAT_TEMP, 'all', function () {
                        return [];
                    });
                    $stat[] = $data;
                    $this->ci->cache->set(TABLE_BANNERS_STAT_TEMP, 'all', $stat, function () use ($data) {
                        $ci = &get_instance();
    ...
    {
        $this->ci->db->delete(TABLE_BANNERS_STAT_GENERAL, ['banner_id' => $banner_id]);
        $this->ci->db->delete(TABLE_BANNERS_STAT_HOURLY, ['banner_id' => $banner_id]);
        $this->ci->db->delete(TABLE_BANNERS_STAT_TEMP, ['banner_id' => $banner_id]);
    }
