...
use Pg\modules\install\models\InstallGeneratorModel;
use Pg\modules\install\models\UpdaterBuilderModel;
...
class AdminInstall extends \Controller
{
    ...
    public const LICENSE = 'https://www.pilotgroup.net/license-agreement';
    ...
    public function index(string $name = '')
    {
        if (!INSTALL_MODULE_DONE) {
            $this->view->setRedirect(site_url() . 'admin/install/copyrights');
        } elseif (INSTALL_MODULE_DONE && !INSTALL_DONE) {
            $this->view->setRedirect(site_url() . 'admin/install/product_install');
        } elseif (INSTALL_DONE) {
            $this->view->setRedirect(site_url() . 'admin/install/login');
        }
    }
    ...
    public function copyrights()
    {
        ...
        if (!empty($this->autoinstall['license_agree'])) {
            $this->view->setRedirect(site_url() . 'admin/install/install_admin');
        }
        ...
        if ($save_button || $skip_button || $is_autoinstall) {
            ...
            if ($save_button) {
                ...
                foreach (['host', 'path', 'user', 'password'] as $name) {
                    $data['ftp_' . $name] = $this->autoinstall['ftp'][$name] ?? '';
                }
                $save_button = true;
            } else {
    ...
    public function installDatabase()
    {
        ...
        if (INSTALL_MODULE_DONE) {
            $this->view->setRedirect(site_url() . 'admin/install/install_langs');
        }
        ...
        if ($is_save || $is_autoinstall) {
            if ($is_save) {
                ...
                foreach (['host', 'user', 'password', 'name', 'prefix'] as $name) {
                    $data['db_' . $name] = $this->autoinstall['database'][$name] ?? '';
                }
                ...
                if (!$data['db_host']) {
                    ...
                    if (!$DB->isConnect()) {
                        ...
                        if (!$return) {
                            ...
                            $this->view->setRedirect(site_url() . 'admin/install/sql');
                        }
                    }
                }
            }
    ...
    public function sql()
    {
        ...
        putenv("SET_MODULE_ACTION=install");
        $this->load->model('install/models/InstallMigrateModel');
        $migrate = $this->InstallMigrateModel->setModule('install')->execute('migrate', ENVIRONMENT, '0');
        $errors = !empty($migrate['errors']) ? $migrate['errors'] : [];
        $seed = $this->InstallMigrateModel->setModule('install')->execute('seed:run', ENVIRONMENT, '0');
        $errors = !empty($seed['errors']) ? array_merge($seed['errors'], $errors) : $errors;
    ...
    {
        if (empty($module_gid)) {
            $this->view->setRedirect(site_url() . 'admin/install/modules');
        }
    ...
    public function langs($action = null, $lang = null, $module = null)
    {
        ...
        switch ($action) {
            case 'install':
                if ((bool)$this->pg_language->get_lang_by_code($lang)) {
                    ...
                    $this->view->setRedirect(site_url() . 'admin/install/langs');
                }
                ...
                if (is_array($data)) {
                    ...
                    $this->view->setRedirect(site_url() . 'admin/install/langs_update/' . $lang_id);
                } else {
            ...
            case 'update':
                if ($lang) {
                    $this->view->setRedirect(site_url() . 'admin/install/langs_update/' . $lang);
                } else {
        ...
        $inst_langs_by_code = $available_langs_by_code = $langs = [];
        ...
        if ($this->input->post('btn_login')) {
            ...
            if (!($data['login'] == $install_login && $install_password == $data['password'])) {
                ...
                if ($this->session->userdata('auth_type') == 'admin') {
                    $this->view->setRedirect(site_url() . 'admin/start/mod_login');
                } else {
                    $this->view->setRedirect(site_url() . 'admin/install');
                }
            } else {
    ...
    {
        $content = $this->checkOverallProductUpdate($file, $path);
    ...
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
        $module_dependencies = $module_data['dependencies'] ?? [];
        ...
        if ((isset($dependencies['error']) && !empty($dependencies['error'])) || (isset($libraries_required['error']) && !empty($libraries_required['error']))) {
            ...
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_dependencies'));
        } else {
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_permissions'));
        }
    ...
    public function ajaxPermissions($module_gid, $install_type = 'product')
    {
        ...
        if (!empty($errors)) {
            ...
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_requirements'));
        }
    ...
    public function ajaxRequirements($module_gid, $install_type = 'product')
    {
        ...
        if ($requirements['result'] == false) {
            ...
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_sql'));
        }
    ...
    public function ajaxSql($module_gid, $install_type = 'product')
    {
        ...
        if ($errors) {
            ...
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_linked'));
        }
    ...
    public function ajaxLinked($module_gid)
    {
        ...
        if ($errors) {
            ...
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_settings'));
        }
    ...
    public function ajaxSettings($module_gid, $install_type = 'product')
    {
        ...
        if ($settings) {
            ...
            $this->view->assign('current_module_percent', $this->getSetPercent('ajax_demo_content'));
    ...
    public function ajaxDemoContent($module_gid, $install_type = 'product')
    {
        ...
        $this->view->assign('errors', $errors);
        $this->view->assign('current_module_percent', ($errors ? $this->getSetPercent('ajax_demo_content') : 100));
    ...
    {
        try {
            $html = HtmlDomParser::file_get_html(self::LICENSE);

            return $html->find('#tab-en', 0)->find('.row', 1)->text();
        } catch (Exception $e) {
    ...
    public function _product_install_libraries()
    {
        ...
        foreach ($product_libraries as $gid) {
            if (!isset($libraries[$gid])) {
                ...
                if (!empty($lib_config)) {
                    $data = ['gid' => $gid, 'name' => $lib_config['name'], 'date_add' => date('Y-m-d H:i:s')];
     ...
     * @return void
     */
    private function productUpdateLibrariesSetup(string $file, string $path = 'product_updates')
    {
        $this->load->model('install/models/Updates_model');
        $this->load->model('install/models/Libraries_model');
        $this->load->library('pg_library');

        $update_config = $this->Updates_model->getUpdateProductConfig($file, $path);
    ...
    public function productUpdateOtherFiles()
    {
        $this->load->model('install/models/UpdatesModel');
        $other_data = $this->UpdatesModel->getUpdateConfig(UpdaterBuilderModel::OTHER_DIR);
        $this->UpdatesModel->copyAndBackupFiles([
            'files' => $other_data['files'],
            'path' => UpdaterBuilderModel::OTHER_DIR
        ]);
    }
     ...
     * @return string
     */
    public function checkOverallProductUpdate($file, $path = 'product_updates'): string
    {
        $this->load->model('install/models/Updates_model');
        $upd_conf = $this->Updates_model->getUpdateProductConfig($file, $path);
        $installed_modules = $this->Install_model->get_installed_modules();
        ...
        if ($allow_to_update) {
            ...
            $this->load->model('install/models/BackupModel');
            $this->BackupModel->createBackupFileInfo();
            $this->Updates_model->updateProductInformation($upd_conf['version_to']);
            $this->view->assign('current_overall_percent', 100);
            $this->load->model('install/models/UpdaterBuilderModel');
            $this->UpdaterBuilderModel->deleteUpdater();
            $this->view->assign('upd_conf', $upd_conf);
    ...
    public function _check_if_module_updated($module_gid, $path = '')
    {
        ...
        $upd = $this->Updates_model->getUpdateConfig($module_gid, $path);

        return empty($upd) || floatval($inst['version']) >= floatval($upd['version_to']);
    }
        ...
        foreach ($dependencies as $gid => $req_data) {
            ...
            if (isset($installed_modules[$gid]) && !empty($installed_modules[$gid])) {
                if ($version > floatval($installed_modules[$gid]['version'])) {
                    $return['error'][] = ['module_gid' => $gid, 'module_version' => $version, 'info' => 'installed version: ' . $installed_modules[$gid]['version'] . '(required version: ' . $version . ')'];
                }
            } elseif ($product_check && in_array($gid, $product_modules)) {
                if ($version > floatval($module_data['version'])) {
                    $return['error'][] = ['module_gid' => $gid, 'module_version' => $version, 'info' => 'installed (available) version: ' . $module_data['version']];
                }
            } elseif (!empty($module_data) && $version <= floatval($module_data['version'])) {
                $return['error'][] = ['module_gid' => $gid, 'module_version' => $version, 'info' => 'please install this module first'];
            } else {
                $return['error'][] = ['module_gid' => $gid, 'module_version' => $version, 'info' => 'the module is not found or does not match the required version(' . $version . ')'];
            }
        }
    ...
    public function _check_required_libraries($dependencies)
    {
        ...
        foreach ($dependencies as $gid) {
            if (isset($installed_libraries[$gid])) {
                $return['info'][] = ['gid' => $gid, 'name' => $installed_libraries[$gid]['name']];
            } else {
                ...
                $return['error'][] = ['gid' => $gid, 'name' => $name];
            }
        }
    ...
    public function _check_files($files)
    {
        ...
        foreach ($files as $file_data) {
            ...
            if (!file_exists($file_path)) {
                $errors[] = ['file' => $file, 'msg' => 'File does not exist'];
            } else {
                if ($type === 'file') {
                ...
                if ($perm === 'write' && !$writeable) {
                    ...
                    if ($ftp_perm === false) {
                        $errors[] = ['file' => $file, 'msg' => 'Please set 777 permissions'];
                    }
                }
            }
        }
    ...
    {
        $this->load->model('install/models/UpdatesModel');
        $updates = $this->UpdatesModel->getEnabledUpdates();
    ...
    {
        $this->load->model('install/models/UpdatesModel');
        $updates = $this->UpdatesModel->getEnabledProductUpdates();
        $this->view->assign('updates', $updates);
    ...
    {
        $this->load->model('install/models/Updates_model');
        $this->productUpdateLibrariesSetup($file . '.php');
        $this->productUpdateOtherFiles();
        $this->view->assign('start_html', $this->checkOverallProductUpdate($file . '.php'));

        $this->view->setHeader('Modules management : Product update');
        $this->view->assign('modules_setup', true);
        $this->view->assign('file', $file . '.php');
    ...
    public function updateInstall($module_gid, $path = '')
    {
        ...
        $update = $this->Updates_model->getUpdateConfig($path);
        ...
        if ($module['version'] != $update['version_from']) {
            $this->view->setRedirect(site_url() . 'admin/install/update_install_chmod/' . $module_gid);
        }
        ...
        $files_changes = $this->Updates_model->getModuleChangedFiles($module_gid);
    ...
    {
        $this->load->model('install/models/Updates_model');
        $module_data = $this->Updates_model->getUpdateConfig($module_gid);

        $errors = $this->Updates_model->migrate($module_gid);
        if ($errors) {
            $this->view->assign('errors', $errors);
    ...
    public function ajaxFiles($module_gid, $install_type = 'product')
    {
        $this->load->model('install/models/UpdatesModel');
        $path = $module_gid;
        $module_data = $this->UpdatesModel->getUpdateConfig($path);
        if (empty($module_data['files'])) {
            $this->view->assign('skip', true);
        } else {
            $ftp_errors[] = $this->UpdatesModel->copyAndBackupFiles([
    ...
    public function ajaxUpdateSettings($module_gid, $install_type = 'product')
    {
        ...
        $module_data = $this->Updates_model->getUpdateConfig($module_gid);
        $path = $module_gid;
        $module_gid = $this->Updates_model->getModuleByPath($path);
        $this->Updates_model->updateLanguageInstall($module_gid, $path);
        $this->Updates_model->updateSettingsInstall($module_gid, $path);
        $this->Updates_model->updatePermissionsInstall($module_gid, $path);
        //TODO (nsavanaev) ModuleUpdateModel
        //$this->Updates_model->updateArbitraryInstall($module_gid, $path);
        $error = '';
        if ($error) {
    ...
    public function ajaxUpdatePublic($module_gid, $install_type = 'product', $file = '', $update_path = 'product_updates')
    {
        ...
        $path = $module_gid;
        $module_gid = $this->Updates_model->getModuleByPath($path);
        $module_data = $this->Updates_model->getUpdateConfig($module_gid, $path);
        $this->Updates_model->migrate($module_gid);
        $data['version'] = $module_data['version_to'];
        $this->Updates_model->updateModuleInformation($module_gid, $data);
        if ($install_type == 'product_update') {
            $content = $this->checkOverallProductUpdate($file, $update_path);
        } else {
    ...
    public function libraryInstall($gid)
    {
        ...
        $this->view->setRedirect(site_url() . 'admin/install/libraries');
    }
            ...
            if (!$data['config_writeable']) {
                ...
                if (!preg_match($login_expr, $data['config_login'])) {
                    ...
                    if (!$return) {
                        ...
                        $this->view->setRedirect(site_url() . 'admin/install/installer_settings');
                    }
                }
            }
        } else {
     ...
     * @return float
     */
    private function getSetPercent(string $step): float
    {
     ...
     * @return float
     */
    private function getUpdateStepPercent(string $step): float
    {
     ...
     * @return void
     */
    public function migrate(string $command = 'status', string $environment = null, string $target = null)
    {
        $this->load->model('install/models/InstallMigrateModel');
        $result = $this->InstallMigrateModel->execute($command, $environment, $target);
     ...
     * @return void
     */
    public function productUpdateDownload(string $version)
    {
        if (!empty($version)) {
            $this->load->model('install/models/UpdaterDownloaderModel');
            $this->UpdaterDownloaderModel->downloadToServer($version);
        }

        $this->view->setRedirect(site_url('admin/install/product_updates'));
    }

    public function migrations()
    {
        $this->load->model('install/models/InstallMigrateModel');
        if (!empty($this->input->post('install_module_migrate'))) {
            $target = $this->input->post('migration_id');
            $result = $this->InstallMigrateModel->execute('migrate', ENVIRONMENT, $target);
            $this->view->assign(nl2br($result['output']));
            $this->view->render();

            return;
        } elseif (!empty($this->input->post('install_all_migrates'))) {
            $result = $this->InstallMigrateModel->execute('migrate', ENVIRONMENT);
            if (!empty($result['errors'])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $result['errors']);
            }
            $this->ci->view->setRedirect(site_url() . 'admin/install/migrations');
        }

        $migrate_modules = $this->InstallMigrateModel->getMigrationsList();
        $this->view->assign('migrate_modules', $migrate_modules);
        $this->view->assign('step', 'migrations');
        $this->view->setHeader('Migrations');
        $this->view->assign('modules_setup', true);
        $this->view->render('migrations');
    }

    public function generator()
    {
        $this->view->assign('step', 'generator');
        $this->view->setHeader('Generator');
        $this->view->assign('modules_setup', true);
    ...
    public function generateModule()
    {
        if (!empty($this->input->post('btn_save'))) {
            ...
            $modules_list = $this->Install_model->get_product_setup_modules();
            if (empty($module_name)) {
                $result['errors'][] = 'Module name not specified!!!';
            } elseif (in_array($module_name, $modules_list, false) === true) {
                $result['errors'][] = 'Module already exists!!!';
            } else {
                $generator_model = new InstallGeneratorModel();
                $generator_model->setModule($module_name);
                $result = $generator_model->generateModule();
            }

            $this->view->assign($result);
            $this->view->render();
        } else {
            $this->view->assign('step', 'generate_module');
            $this->view->setHeader('Generate module');
            $this->view->assign('modules_setup', true);
            $this->view->render('generate_module');
        }
    }
    ...
    public function generateCrud()
    {
        if (!empty($this->input->post('btn_save'))) {
            ...
            $model_name = $this->input->post('model', true);
            $controller_side = $this->input->post('controller_side', true);
            $generator_model = new InstallGeneratorModel();
            $generator_model->setModule($module_name);
            $generator_model->setModel($model_name);
            $generator_model->setTable($table_name);
            $generator_model->setControllerSide($controller_side);
            $generator_model->setFields($this->db->field_data($table_name));
            $generator_model->setFormatNameModel(false);
            $result = $generator_model->generateCrud();
            $this->view->assign($result);
            $this->view->render();
        } else {
            $modules_list = $this->Install_model->get_product_setup_modules();
            ...
            $this->view->assign('list_tables', $list_tables);

            $roles = ['admin', 'api', 'user'];
            $this->view->assign('roles', $roles);

            $this->view->assign('step', 'crud');
            $this->view->setHeader('Generate CRUD');
            $this->view->assign('modules_setup', true);
            $this->view->render('crud');
        }
    }

    public function exampleModelName()
    {
        $result = [];
        if (!empty($this->input->post('btn_save'))) {
            $module = $this->input->post('module', true);
            $table = $this->input->post('table', true);
            $result['data']['model'] = (new InstallGeneratorModel())->formatNameModel($module, str_replace(DB_PREFIX, '', $table));
        }
    ...
    public function generateModel()
    {
        if (!empty($this->input->post('btn_save'))) {
            ...
            $generator_model->setFields($this->db->field_data($table_name));
            $generator_model->setFormatNameModel(false);
            $result = $generator_model->generateModel();
            $this->view->assign($result);
            $this->view->render();
        } else {
            $modules_list = $this->Install_model->get_product_setup_modules();
            ...
            $this->view->assign('step', 'generate_model');
            $this->view->setHeader('Generate model');
            $this->view->assign('modules_setup', true);
            $this->view->render('generate_model');
        }
    }
}
