...
class AdminPolls extends \Controller
{
    ...
    public function index($filter = 'all', $order = 'question', $order_direction = 'ASC', $page = 1)
    {
        ...
        $attrs            = $search_params    = [];
        // Грузим настройки
        $current_settings = isset($_SESSION["polls_list"]) ? $_SESSION["polls_list"] : [];
        ...
        if ($polls_count > 0) {
            $polls      = $this->Polls_model->get_polls_list($page, $items_on_page, [$order => $order_direction], $attrs);
            //Select user_type
    ...
    public function edit($id = 0)
    {
        ...
        if(!empty($poll_data['use_expiration']) && !empty($poll_data['date_end'])){
            $poll_data['date_end']   = date('d-m-Y', strtotime($poll_data['date_end']));
        }else{
            $poll_data['date_end'] = '00-00-0000';
        }
        $poll_data['date_start'] = !empty($poll_data['date_start']) ? date('d-m-Y', strtotime($poll_data['date_start'])) : date('d-m-Y');
    ...
    public function answers($id = 0)
    {
        ...
        if ($poll_data && $this->input->post('btn_save', true)) {
            ...
            if (empty($post_data['answers_colors']) || count($post_data['answers_colors']) == 0) {
                $this->Polls_model->activate_polls($id, 0);
            }
            ...
            $this->Polls_model->checkTable(count($poll_data['answers_colors']));
    ...
    public function activate($poll_id, $status = 0)
    {
        if (!empty($poll_id)) {
            ...
            if (empty($poll_data['answers_colors']) || count($poll_data['answers_colors']) == 0) {
                $this->system_messages->addMessage(View::MSG_ERROR, l('error_answer_version_empty', 'polls'));
            } else {
    ...
    public function results($poll_id = 0, $filter = 'all', $order = 'user_id', $order_direction = 'ASC', $page = 1)
    {
        ...
        if (empty($poll_data['answers_colors']) || count($poll_data['answers_colors']) == 0) {
            ...
            $attrs = $search_params = [];
            // Загружаем настройки
            $current_settings = isset($_SESSION["results_list"]) ? $_SESSION["results_list"] : [];
            if (!isset($current_settings["filter"])) {
                $current_settings["filter"] = $filter;
            }
            if (!isset($current_settings["order"])) {
