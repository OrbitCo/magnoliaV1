...
class AdminSubscriptions extends \Controller
{
    ...
    public function index($order = "subscribe_type", $order_direction = "DESC", $page = 1)
    {
        ...
        $attrs = [];
        $current_settings = isset($_SESSION["subscriptions_list"]) ? $_SESSION["subscriptions_list"] : [];
        ...
        if ($subscriptions_count > 0) {
            $subscriptions = $this->Subscriptions_model->get_subscriptions_list($page, $items_on_page, [$order => $order_direction], $attrs);
    ...
    {
        $this->load->model('Subscriptions_model');
        $data = ($id) ? $this->Subscriptions_model->get_subscription_by_id($id) : [];
