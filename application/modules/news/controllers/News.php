...
class News extends \Controller
{
    ...
    public function index($page = 1)
    {
        ...
        if ($news_count > 0) {
            $news = $this->News_model->get_news_list($page, $items_on_page, ['date_add' => "DESC"], $attrs);
    ...
    public function view($id)
    {
        ...
        if ($lang_canonical) {
            ...
            if ($lang_id != $this->pg_language->current_lang_id) {
                $news_canonical = $this->News_model->get_news_by_id($id, [$lang_id]);
    ...
    public function rss()
    {
        ...
        $news = $this->News_model->get_news_list(1, $rss_settings["rss_news_max_count"], ['date_add' => "DESC"], $attrs);
        if (!empty($news)) {
            $this->load->helper('seo');
