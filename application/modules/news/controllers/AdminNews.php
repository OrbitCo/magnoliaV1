...
class AdminNews extends \Controller
{
    ...
    public function index($order = "date_add", $order_direction = "DESC", $page = 1)
    {
        ...
        $current_settings = !empty($_SESSION["news_list"]) ? $_SESSION["news_list"] : [];
        ...
        if ($news_count > 0) {
            $news = $this->News_model->get_news_list($page, $items_on_page, [$order => $order_direction], $attrs);
    ...
    public function edit($news_id = null, $section_gid = 'text')
    {
        ...
        switch ($section_gid) {
            case 'text':
                ...
                foreach ($languages as $lid => $lang_data) {
                    if (!empty($data['content']) && is_array($data['content']) && empty($data['content_' . $lid])) {
                        if ($data['content'][$lid]) {
                    ...
                    if (!empty($data['name']) && is_array($data['name']) && empty($data['name_' . $lid])) {
                        if ($data['name'][$lid]) {
                            $data['name_' . $lid] = $data['name'][$lid];
                        }
                    }
                    if (!empty($data['annotation']) && is_array($data['annotation']) && empty($data['annotation_' . $lid])) {
    ...
    public function feeds($id_lang = 0, $order = "date_add", $order_direction = "DESC", $page = 1)
    {
        ...
        $current_settings = isset($_SESSION["feeds_list"]) ? $_SESSION["feeds_list"] : [];
        ...
        if ($feeds_count > 0) {
            $feeds = $this->Feeds_model->get_feeds_list($page, $items_on_page, [$order => $order_direction], $attrs);
