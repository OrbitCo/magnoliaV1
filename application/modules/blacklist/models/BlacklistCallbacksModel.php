...
class BlacklistCallbacksModel extends \Model
{
    ...
    {
        if (empty($event_status) && !empty($module)) {
            $fields     = $this->fields_str;
            $nameTable  = BLACKLIST_CALLBACKS_TABLE;
            $result = $this->ci->cache->get(self::CACHE_NAME, 'getCallbacks' . $module, function () use ($module, $fields, $nameTable) {
                $ci = &get_instance();
