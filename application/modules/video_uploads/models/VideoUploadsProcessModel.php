...
class VideoUploadsProcessModel extends \Model
{
    ...
    public function prepare($file_name, $file_path, $file_data, $id_object, $video_upload_gid)
    {
        ...
        $this->getCallback($config["module"], $config["model"], $config["method_status"], $id_object, $status, $callback_data);
    }
    ...
    public function cronProcessingMethod()
    {
        ...
        while ($this->getTimer('processing') < $this->cron_time_limit) {
            //// get process
            $process = $this->getProcessByStatus('start');
            ...
            $return = [];
            if ($config["upload_type"] == 'local') {
                $return = $this->ci->Video_uploads_local_model->processingMethod($process["file_name"], $process["file_path"], $process["file_data"], $config);
            }

            if ($config["upload_type"] == 'youtube') {
                $return = $this->ci->Video_uploads_youtube_model->processingMethod($process["file_name"], $process["file_path"], $process["file_data"], $config);
            }
            //// if errors
            if (!empty($return["errors"])) {
                ...
                if (!empty($data['image'])) {
                    $this->ci->Video_uploads_model->createThumbs($data['image'], $process["file_path"], $config["thumbs_settings"]);
                }
            }
            ...
            if ($status == "end") {
                ...
                $this->saveProcess($process['id'], $process_data);
            }
     ...
     */
    public function getProcessList($params = [], $order_by = null, $items_on_page = null, $page = 1)
    {
    ...
    public function getProcessCount($params = [])
    {
     ...
     */
    public function getCallback($module, $model, $method, $id_object, $status, $data = [], $errors = [])
    {
        $model_url = $module . "/models/" . $model;
        $model_path = MODULEPATH . strtolower($model_url) . EXT;
        $this->ci->load->model($model_url);
        $function_result = call_user_func_array([&$this->ci->{$model}, $method], [$id_object, $status, $data, $errors]);
