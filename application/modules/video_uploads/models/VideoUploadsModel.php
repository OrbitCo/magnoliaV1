...
class VideoUploadsModel extends \Model
{
    ...
    /**
     * Wall event video GID
     * @var string
     */
    const WALL_EVENT_GID = 'video_upload';
    ...
    public function upload($config_gid, $postfix, $upload_gid, $object_id, $file_data, $name_format = 'format')
    {
        ...
        if (!$type_check || !isset($_FILES[$upload_gid]['name'])) {
            ...
            switch ($name_format) {
                case "generate":
                    $new_file_name = $this->generate_filename($file_name_ext);
                    break;
                case "format":
                default:
    ...
    public function formatThumbs($file_name, $url, $path, $thumbs_data)
    {
        ...
        foreach ($thumbs_data as $thumb_data) {
            ...
            $upload["thumbs_data"][$thumb_data["gid"]] = [
    ...
    ////// graph function
    private function action($file, $thumb = [], $dynamic_output = false)
    {
    ...
    private function resize($file, $width, $height, $type = 'in', $maintain = false, $color = false, $dynamic_output = false)
    {
        ...
        switch ($type) {
            case "in":
                $master = ($k1 <= $k) ? "height" : "width";
                break;
            case "out":
                $master = ($k1 <= $k) ? "width" : "height";
                break;
        }
