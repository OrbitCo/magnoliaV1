    ...
    public function keepAlive($listen = false)
    {
        if ($listen) {
            ...
            $this->listenSocket->on('event', function ($data) {
                if (!empty($data['event'])) {
                    $this->emit($data['event'], (!empty($data['data'])) ? $data['data'] : [], $this->namespace);
                }
        ...
        $this->emit('saved', [], $this->namespace);
        ...
        while (is_resource($this->fd)) {
            ...
            if ($this->session['heartbeat_timeout'] > 0 && $timeout + $this->heartbeatStamp - 5 < $this->mctime()) {
                ...
                $this->emit('heartbeat', [], $this->namespace);
            }
    ...
    private function handshake()
    {
        try {
            ...
            if (!is_null($this->handshakeTimeout)) {
                ...
                $constants = [CURLOPT_CONNECTTIMEOUT_MS, CURLOPT_TIMEOUT_MS];
                ...
                if (true === version_compare($version, '7.16.2', '<')) {
                    $timeout  /= 1000;
                    $constants = [CURLOPT_CONNECTTIMEOUT, CURLOPT_TIMEOUT];
                }
