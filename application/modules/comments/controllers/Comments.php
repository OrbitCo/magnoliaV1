...
class Comments extends \Controller
{
    ...
    public function ajaxAddComment()
    {
        ...
        if (
            $comments_type['status'] && $this->session->userdata('auth_type') != 'admin'
    ...
    public function ajaxDeleteComment()
    {
        ...
        if ($result['comments']) {
            ...
            if (
                $comments_type && $comments_type['status']
    ...
    public function ajaxLoadComments()
    {
        ...
        if ($result['comments']['comments'] || $with_form) {
            ...
            $user = $this->Users_model->get_users_list(null, null, null, null, [$this->session->userdata('user_id')]);
    ...
    public function ajaxLikeComment()
    {
        ...
        $comment = isset($comments['comments'][0]) ? $comments['comments'][0] : [];

        $comments_type = [];
        $comments_type = $comment ? $this->Comments_types_model->get_comments_type_by_gid($comment['gid']) : [];
