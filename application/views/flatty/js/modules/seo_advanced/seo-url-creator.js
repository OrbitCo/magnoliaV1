function seoUrlCreator(optionArr)
{
    ...
    this.create_url = function () {
        ...
        for (var part in _self.properties.data) {
            if (_self.properties.data[part].type == 'postfix') {
                postfix = true;
            }
            if (postfix && _self.properties.data[part].type == 'text') {
                _self.properties.data[part].type = 'postfix_text';
            }
    ...
    this.save_li = function (id, data) {
        ...
        if (!id) {
            if (_self.id > 0 && data.type != 'postfix' && data.type != 'postfix_text') {
                liClass += ' sortable';
            }
            id = 'li' + (++_self.id);
            ...
            $('#' + id).bind('click', function () {
                if ($('#' + id).find('input,select').length > 0 || !_self.properties.showForm) {
                    return;
                }
    ...
    this.refresh_data = function () {
        ...
        $('#' + _self.properties.urlID + '> li').each(function (i, item) {
            if (item.getAttribute('class') == _self.properties.fixedClass + ' url-block') {
                return;
            }
        ...
        switch (type) {
            ...
            case 'postfix':
                ...
                for (var i in options_arr) {
                    options_str += '<option ' + (i == var_name ? 'selected' : '') + '>' + i + '</option>';
                }
    ...
    this.save_form = function (id) {
        ...
        switch (type) {
            ...
            case 'postfix_text':
                ...
                if (text.length <= 0) {
                    _self.cancel_form(id); return false;}
                var data = {
                    type: 'text',
                    value: text,
                    var_num: '',
    ...
    this.serialize_data = function () {
        ...
        for (var id in _self.properties.data ) {
            if (_self.properties.data[id].type == 'postfix_text') {
                _self.properties.data[id].type = 'text'
                ret += '"' + index + '":{';
            }
