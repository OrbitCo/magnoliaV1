...
function Associations(optionArr)
{
    ...
    this.Init = function (options) {
        ...
        if (_self.properties.template && typeof (twig) === 'function') {
            twig({
                id: "association" + _self.properties.rand,
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (typeof data.errors !== 'undefined' && data.errors.length > 0) {
                        _self.properties.errorObj.show_error_block(data.errors, 'info');
                        return false;
                    } else if (typeof data.info.access_denied !== 'undefined' && data.info.access_denied.length > 0) {
                        if (_self.properties.new_tab) {
                            locationHref(_self.properties.siteUrl + 'access_permissions', true, true);
                        }
    ...
    this.viewAssociation = function (key) {
        ...
        if (check_image === true) {
            ...
            $('#' + _self.properties.association_content_id).find('img.img-rounded').on('load', function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data.errors) {
                    ...
                    sendAnalytics('associations_send_association', 'communication', 'user');
                } else {
