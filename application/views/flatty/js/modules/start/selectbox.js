function selectBox(optionArr) {
    ...
    this.initBg = function () {
        if ($('body').find('#select_box_bg').length) {
            return;
        }
    ...
    this.expandBg = function (box_id) {
        $('#select_box_bg').show().unbind('click').on('click', function () {
    ...
    this.initBox = function (box_id) {
        if ($('#' + box_id + '_box').length < 1 || $('#' + box_id + '_box[sb_init="1"]').length)
            return;
        $('#' + box_id + '_box').attr('sb_init', '1');
        _self.createDropDown(box_id);
        $('#' + box_id + '_box, #' + box_id + '_dropdown').unbind('click').on('click', function (e) {
    ...
    this.openBox = function (box_id) {
        ...
        $('body').unbind('click.selectBox').on('click.selectBox', function (e) {
            _self.closeBox(box_id);
        });
    };
