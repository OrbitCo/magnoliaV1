function requestSelect(optionArr)
{
    this.properties = {
        siteUrl: '',
        rand: '',
        load_user_link: 'companions/ajaxGetRequestsData/',
        load_form: 'companions/ajaxGetRequtestForm/',
        load_selected_data_link: 'companions/ajaxGetSelectedRequest/',
        id_main: '',
        id_span: '',
        id_manage_link: '',
    ...
    this.Init = function (options) {
        ...
        $('#' + _self.properties.id_manage_link).on('click', function () {
            _self.open_form();
            return false;
        });
    };
    ...
    this.open_form = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                _self.load_users('', 1);
                $('#' + _self.properties.id_search).unbind().on('keyup', function () {
                    _self.load_users($(this).val(), 1);
                });

                if (_self.properties.max != 1) {
                    $('#' + _self.properties.id_selected_items + ' input:checkbox').on('click', function () {
                        _self.unset_user($(this).val());
                    });
                }

                $('#' + _self.properties.id_close).on('click', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var id in data.items ) {
                    ...
                    elem += '>' + data.items[id].title + '(' + data.items[id].user.output_name + ')</li>';
                    $('#' + _self.properties.id_items).append(elem);
                }
                _self.generate_user_pages(data.pages, data.current_page, search);
                $('#' + _self.properties.id_items + ' li').on('click', function () {
    ...
    this.generate_user_pages = function (pages, current_page, search) {
        ...
        if (pages > 1) {
            ...
            $('#' + _self.properties.id_user_page + ' a').on('click', function () {
...
//                  _self.load_users(search, page);
    ...
    this.reload_span = function (data) {
        ...
        for (var i in data) {
            if (_self.properties.max != 1) {
                $('#' + _self.properties.id_span).append(data[i].title + '(' + data[i].user.output_name + ')<br><input type="hidden" name="' + _self.properties.var_name + '[]" value="' + data[i].id + '">');
            } else {
                $('#' + _self.properties.id_span).append(data[i].title + '(' + data[i].user.output_name + ')<input type="hidden" name="' + _self.properties.var_name + '" value="' + data[i].id + '">');
            }
        }
    };
    ...
    this.reload_selected = function (data) {
        ...
        for (var i in data) {
            $('#' + _self.properties.id_selected_items).append('<li><div class="user-block"><input type="checkbox" name="remove_users[]" value="' + data[i].id + '" checked>' + data[i].title + '(' + data[i].user.output_name + ')</div></li>');
        }
        $('#' + _self.properties.id_selected_items + ' input:checkbox').on('click', function () {
            _self.unset_user($(this).val());
        });
...
if (typeof exports === 'object') {
    ...
    exports.requestSelect = requestSelect;
}
