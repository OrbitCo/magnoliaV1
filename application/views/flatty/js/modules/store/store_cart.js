...
function storeCart(optionArr)
{
            ...
            _request(_self.properties.removeUrl, '', items, 'json', function (data) {
                ...
                $(_self.properties.cartBlock + ' input[type=checkbox]').each(function () {
                    ...
                    if (!$(this).prop('checked')) {
                        ...
                        $(_self.properties.cartItem + item).hide();
                    }
                });
    ...
    this.preorder = function () {
        ...
        $(_self.properties.cartBlock + ' input[type=checkbox]').each(function () {
            if ($(this).prop('checked')) {
                ...
                $(_self.properties.cartItem + $(this).data('cartitem')).remove();
            }
        });
        _self.setTotalSum(totalSum);
        $(_self.properties.cartCount).html(totalCount);
    ...
    this.savePreorder = function () {
        ...
        $(_self.properties.cartBlock + ' input[type=checkbox]').each(function () {
            if (!$(this).prop('checked')) {
                $(_self.properties.cartItem + $(this).data('cartitem')).hide();
            }
        });
    ...
    this.getItemSum = function (id) {
        return parseInt($(_self.properties.cartItemPrice + id).data('price'));
    };

    this.getTotalSum = function () {
        var total = parseInt($(_self.properties.cartTotal).data('total'));
        if (total < 0) {
    ...
    this.setTotalSum = function (sum) {
        $(_self.properties.cartTotal).data('total', sum);
        $(_self.properties.cartTotal).html(
            _self.currencyOutput(sum)
        );
    ...
    this.currencyOutput = function (price) {
        ...
        return price + " " + _self.properties.currency;
    };
